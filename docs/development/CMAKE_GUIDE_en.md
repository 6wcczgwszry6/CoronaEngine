...
- `corona_third_party.cmake`: Consumes Conan-generated package config files and exposes the compatibility targets used by the existing build graph.
...
All external dependencies are resolved by Conan before CMake configure. `misc/cmake/corona_third_party.cmake` calls `find_package(... CONFIG REQUIRED)` for the packages generated by Conan and fails if a required package target is missing. Key dependencies include:
- **Core**: Horizon, ktm, nlohmann_json, TBB
- **Windowing & Rendering**: SDL3, volk, VulkanMemoryAllocator, SPIRV-Cross
- **Asset Loading**: assimp, stb, tinyexr, meshoptimizer, astc-encoder
