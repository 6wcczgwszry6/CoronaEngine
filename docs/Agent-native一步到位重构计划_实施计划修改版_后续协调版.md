> **历史文档提示（2026-07-14）**
> 本文档保留为架构演进与历史实施记录，不再作为当前任务的优先级来源。
...
```text
草地  / ground / terrain 进入 environment / terrain / substrate 链路
帐篷 / 小木桌进入 asset / model / actor 链路
...
## 14. 2026-07-08 后续执行修订：R2.5 / F5 前 Game-ready Scene Runtime 最短攻坚路径

> **本章节是当前最新执行口径。**  
> 若本章节与前文中较长期、较完整的 Phase 计划存在优先级冲突，在第一轮 F5 / 实机验证通过前，优先遵守本章节。  
> 本章节不是推翻原计划，而是把后续执行范围压缩到“能承接 AI Game Demo 的场景 Runtime 最小可运行切片”。

---

### 14.1 最新现实判断

当前系统已经不是单一旧 `SceneComposer` 主链路，但也还不是完全干净的 Agent-native。真实状态是：

```text
AgentRuntime 主控正在接管
旧 workflow / RoleAgent / Coordinator / LANChat planning gate 仍被保护性保留
入口、状态、确认、执行、报告仍存在新旧路径竞争
```

因此，当前最容易出问题的不是“缺少更多 Agent 能力”，而是：
...
后续重构的核心不再是继续扩展功能，而是让 AgentRuntime 真正拥有：
...
```text
R2.5 / F5 前 Game-ready Scene Runtime 最小攻坚
```
...
```text
AI Game Demo 的场景 Runtime 承接层
```

也就是：

```text
生成一个后续游戏逻辑可以消费、查询、引用、回放的场景实体世界。
```
...
```text
用“森林营地”验证通用 outdoor terrain scene generation vertical slice，
而不是把系统写成森林营地专用生成器。
```
...
后续 Codex / AI 执行必须优先沿着这条最短路径推进：

```text
修 AgentRuntime queue drain failed 的真实根因
  ↓
确认生成后至少产生 1 个 ScenePlan / BatchPlan
  ↓
BatchPlan 至少产生 1 个 ToolCallGraph
  ↓
ToolCallGraph 至少执行一个最小 actor.import/create 工具
  ↓
ToolResult 必须提交 StatePatch
  ↓
StatePatch 必须写入 RuntimeState
  ↓
OperationLog 必须能复盘 plan -> terrain -> asset -> actor -> review -> report
  ↓
scene_entity_registry 必须能输出实体清单
  ↓
final report 只能读取 RuntimeState + OperationLog
```
...
#### P0-0：修复 AgentRuntime queue drain failed 的真实根因

目标：

```text
queue drain failed 必须有明确原因、明确状态、明确 OperationLog 记录。
```
...
```text
1. 不再出现无原因 queue drain failed。
2. failed / blocked / cancelled graph 必须进入 terminal 状态。
3. graph 不得永久 queued / running。
4. drain 失败必须进入 OperationLog。
5. final report / status_summary 能读到安全摘要。
6. failed 不能被误算成 completed。
```
...
吞异常
伪装 completed
用 replay summary 掩盖 queue drain 问题
为了修 queue 重写整个 Runtime 队列系统
```

---

#### P0-1：确认生成必须产生 ScenePlan + BatchPlan

目标：

```text
用户确认生成后，RuntimeState 中必须至少出现：
1 个 ScenePlan
1 个 BatchPlan
```
...
```text
active_plan_id 不为空
active_batch_id 或 batch_plans 不为空
BatchPlan 绑定 plan_id
BatchPlan.items 至少包含本轮需求中可执行的场景项
environment items 和 actor items 不混在同一路
```
...
```text
BatchPlan 不是聊天摘要。
BatchPlan 不是 Coordinator SeedPlan。
BatchPlan 必须是 RuntimeState 里的计划事实。
```

---

#### P0-2：BatchPlan 必须产生 ToolCallGraph

目标：

```text
每个可执行 BatchPlan 至少生成一个 ToolCallGraph。
```
...
```text
1. RuntimeState 或 OperationLog 可查询 graph_id。
2. graph 绑定 plan_id / batch_id。
3. graph 至少覆盖 environment / asset / actor / review / report 类节点。
4. graph 可以进入 queue。
5. drain 后状态不能永远 queued。
```

---

#### P0-3：terrain / environment / substrate 路由必须先于普通 asset/model

目标：

```text
草地 / 天空 / ground / terrain / sky / forest floor 等环境项
必须进入 environment / terrain / substrate 链路，
不得进入普通 asset / model / actor 链路。
```

基础分类：

```text
草地 -> terrain/substrate
天空 -> environment/skybox
ground -> terrain/substrate
terrain -> terrain
森林背景 / 树林环境 -> environment
树木个体 / 可摆放木桩 -> asset/model/actor
帐篷 -> asset/model/actor
小木桌 -> asset/model/actor
```

完成标准：

```text
1. environment_state / terrain_state 能记录草地、天空、地形类事实。
2. asset_state / actor_state 不包含天空、草地等环境项的普通模型导入结果。
3. 路由逻辑是通用词类 / entity_type 规则，不是森林营地专用 if。
```

---

#### P0-4：最小 actor.import/create 工具闭环

目标：

```text
ToolCallGraph 至少能执行一个 actor.import/create 类工具。
```
...
除非真实 C++ / Engine 返回了 actor_id / component_id / transform result 等成功结果。

完成标准：

```text
1. actor.import/create 工具被 ToolCallGraph 调用。
2. RuntimeGuard 有授权或拒绝记录。
3. ToolResult 有明确 success / failed / unavailable / pending_f5。
4. ToolResult 只提交 StatePatch，不直接改 RuntimeState。
5. StatePatch 写入 actor_state。
6. OperationLog 能看到 actor import 尝试和结果。
```

---

#### P0-5：actor transform / grounding / AABB 最小事实

目标：

```text
每个 actor 至少有 transform、grounding_status、AABB/bounds 的最小事实。
```
...
1. 帐篷有 transform。
2. 小木桌有 transform。
3. actor 有 grounding_status。
4. actor 有 AABB / bounds。
5. geometry_state 能查到 actor 对应结果。
6. review_state 能看到 geometry / grounding review 摘要。
```
...
```text
把 RuntimeState 中的 terrain / environment / asset / actor / geometry / review / sync
聚合成后续游戏逻辑可消费的实体清单。
```
...
```text
1. scene_entity_registry 能输出草地 / 天空 / 帐篷 / 小木桌相关事实。
2. actor 类实体有 actor_id 或 pending_actor_id。
3. environment 类实体不伪装成 actor。
4. final report 可以读取 entity registry 摘要。
5. 后续 GameDesignPlan / ScriptPlan 可以稳定引用这些字段。
```

---

#### P0-7：final report 只读 RuntimeState + OperationLog

目标：

```text
final report 不再拼 SceneComposer summary，
不读 GenerationScheduler 内部状态，
不读 Coordinator 猜测状态，
不读 RoleAgent chat memory。
```
...
batch 是否创建
ToolCallGraph 是否执行
terrain/environment 是否进入对应链路
asset/model/actor 是否进入对应链路
actor import 是 success / failed / runtime_state_only / unavailable / pending_f5
transform / AABB / grounding 是 resolved / estimated / unknown / failed / pending_f5
scene_entity_registry 输出了多少实体
sync / VLM / real engine 效果哪些是 [待 F5/实机验证]
```
...
除非真实 Engine / RuntimeState / OperationLog 三者一致支持该结论。
...
#### P0-8：入口与 Legacy 静态封锁只做必要收口

当前不进行大规模入口重构，但必须确保：

```text
生成类请求不得漏到 RoleAgent 旧 compose
RoleAgent / MasterAgent 不得返回 start_generation action
SceneComposer / ProgressiveWorkflow 不得重新暴露普通用户入口
旧 workflow 不得被包装成 legacy big tool
状态查询不得读取旧 workflow summary
```

RoleAgent / MasterAgent 当前定位：
...
```text
验证通用 outdoor terrain scene Runtime，
不是验证森林营地专用逻辑。
```
...
草地 / 天空 / ground / terrain 进入 environment / terrain / substrate 链路
帐篷 / 小木桌进入 asset / model / actor 链路
actor import/create 经过 RuntimeGuard
actor 有 transform / grounding / AABB
RuntimeState 能查 terrain / environment / asset / actor / geometry / review
scene_entity_registry 能输出实体清单
OperationLog 能复盘 plan -> terrain -> asset -> actor -> review -> report
final report 只读 RuntimeState + OperationLog
```
...
```text
verify_ultimate_plan.py 失败
语法编译失败
RuntimeGuard 绕过
ToolResult 直接写 RuntimeState
StatePatch 不经 Validator
final report 读取旧 workflow
SceneComposer / ProgressiveWorkflow 普通入口复活
queue drain 无原因失败
engine success 伪造
```
...
### 14.9 与 AI Game Demo 的承接门槛

当前阶段完成后，项目承接 AI Game Demo 的门槛按以下顺序判断：

```text
R2.5：
F5 前最小 vertical slice。
证明 Runtime 能把场景实体事实写入 RuntimeState / OperationLog。
不能正式接游戏 Agent。

R3-min：
Game-ready Scene Runtime 最小可用。
scene_entity_registry 稳定，actor/entity facts 可被后续逻辑消费。
可以开始设计 G0 schema，但不接可执行游戏 Agent。

完整 R3：
Game-ready Scene Runtime 完成。
RuntimeState / scene_entity_registry / OperationLog / Engine write / sync / review 事实稳定。
可以进入 G1：GameDesignAgent / StoryAgent / CombatAgent / BalanceAgent 输出结构化计划。

G2：
Gameplay ToolCallGraph。
可以把 GameDesignPlan / QuestPlan / CombatPlan / BalancePlan 转成 gameplay state facts。

G3：
ScriptAgent / BlueprintAgent。
必须经过 ScriptValidator / RuntimeGuard / EngineScriptAdapter / ToolResult / StatePatch。

G4：
一键 AI Game Demo。
用户输入一个游戏目标，系统生成可玩的 demo。
```

关键边界：

```text
R3 之前：不要正式做游戏 Agent 主链路。
R3-min 之后：可以做 GameWorldState / GameDesignPlan schema。
完整 R3 之后：可以接策划 / 剧情 / 战斗 / 数值 Agent，但只输出 Plan。
G3 之后：才接 ScriptAgent / BlueprintAgent。
```

---

### 14.10 给 AI / Codex 的最新执行口径

后续给 AI / Codex 的任务提示建议统一使用以下口径：

```text
你必须遵守：
E:\corona\CoronaEngine\docs\Agent任务约束循环.md

你必须按以下计划继续推进：
E:\corona\CoronaEngine\docs\Agent-native一步到位重构计划_实施计划修改版.md

当前最新覆盖章节：
第 14 章：2026-07-08 后续执行修订：R2.5 / F5 前 Game-ready Scene Runtime 最短攻坚路径。
...
当前目标：
不是实现完整 AI Game Demo。
不是实现 GameDesignAgent / CombatAgent / StoryAgent / BalanceAgent / ScriptAgent / BlueprintAgent。
本阶段只把场景 Runtime 做到后续游戏逻辑可消费。

最短路径：
1. 修复 AgentRuntime queue drain failed 的真实根因。
2. 确认生成后至少产生 1 个 ScenePlan 和 1 个 BatchPlan。
3. BatchPlan 至少产生 1 个 ToolCallGraph。
4. ToolCallGraph 至少执行 terrain/environment route 与 actor.import/create 最小工具。
5. 所有真实写入必须走 ToolCall -> RuntimeGuard -> EngineWriteGate / runtime_cpp_bridge -> ToolResult -> StatePatch -> RuntimeState -> OperationLog。
6. scene_entity_registry 必须输出后续游戏逻辑可消费的实体清单。
7. final report 只能读取 RuntimeState + OperationLog。
8. C++ actor import / transform / delete、terrain 真实写入、sync、asset transfer、VLM screenshot、真实 Engine 效果统一标记 [待 F5/实机验证]。

禁止：
- 绕过 RuntimeGuard；
- 把 SceneComposer / ProgressiveWorkflow 包成 legacy big tool；
- 重新暴露旧 workflow 用户入口；
- 为森林营地、帐篷、小木桌、草地、天空写死专用逻辑；
- 当前阶段实现上层游戏 Agent；
- 伪造 Engine success；
- 为非 P0 边角测试大面积重构；
- 扩大量 replay summary / 门禁 / 测试矩阵。
```

---

### 14.11 本章节完成后，下一步应该进入的任务队列

推荐后续任务队列顺序：

```text
Task R2.5-01：
定位并修复 AgentRuntime queue drain failed 真实根因。

Task R2.5-02：
确认生成后强制落 ScenePlan / BatchPlan，并写 RuntimeState + OperationLog。

Task R2.5-03：
BatchPlan 生成 ToolCallGraph，至少包含 scene.extract_objects / terrain route / actor import / review / report。

Task R2.5-04：
terrain / environment / substrate 路由与 asset/model/actor 路由彻底分离。

Task R2.5-05：
最小 actor.import/create 闭环，严格执行 RuntimeGuard -> EngineWriteGate -> ToolResult -> StatePatch。

Task R2.5-06：
actor transform / grounding / AABB 最小事实写入 geometry_state / review_state。

Task R2.5-07：
scene_entity_registry 聚合 terrain / environment / asset / actor / geometry / review / sync。

Task R2.5-08：
final report 改为只读 RuntimeState + OperationLog，并区分 runtime_state_only / pending_f5 / engine_success。

Task R2.5-09：
使用固定 F5 最小场景做实机验证，并把未验证项标记 [待 F5/实机验证]。

Task R2.5-10：
F5 通过后，再决定是否进入 R3-min / GameWorldState schema。
```
...
不要继续补自然语言分类。
不要继续扩上层 Agent。
不要继续堆 replay summary。
不要继续让旧 workflow 参与事实源。

先让 AgentRuntime 能可信地产生：
ScenePlan
BatchPlan
ToolCallGraph
RuntimeState
...
```text
先让 Runtime 可信地产生“游戏可消费的场景实体世界”，再让游戏 Agent 消费这个世界。
```
