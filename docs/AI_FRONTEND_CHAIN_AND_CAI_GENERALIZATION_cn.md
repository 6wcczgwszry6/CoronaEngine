...
- [ ] 保留 `cai_extensions` 目录位置，将其整理为 CabbageEditor adapter。
- [ ] CabbageEditor 专属 workflow 保留在 `cai_extensions/flows`，不进入 CAI 通用核心。
- [ ] CabbageEditor 路径解析、engine tools 只在 adapter 中注册。
- [ ] CAI 通用核心删除对宿主路径的假设。
...
- [ ] 新增 `pyproject.toml`。
- [ ] 拆分 optional dependencies：`langchain`、`workflow`、`media`、`cabbage`。
- [ ] 提供 CLI 示例。
- [ ] 提供 FastAPI/WebSocket 示例。
- [ ] 提供最小 README 和 API reference。
...
- [ ] 将 `cai_extensions.install()` 改造成 `install(app, context)`。
- [ ] 路径 provider 改为 runtime scoped。
- [ ] engine tools 注册到 runtime scoped tool registry。
- [ ] workflow 注册到 runtime scoped workflow registry。
- [ ] 删除对全局 `sys.path` 的不必要依赖。
...
```python
from cai import CAIApp, CAIConfig
from cai_extensions.register import install_cabbage_editor_extension, CabbageContext

config = CAIConfig.from_default_locations()
app = CAIApp(config)

context = CabbageContext.from_editor_runtime()
install_cabbage_editor_extension(app, context)
