...
> 本文档是当前唯一保留的推进计划；已被取代的历史计划已清理。
> 所有 Agent / Codex 任务同时遵守 `docs/Agent任务约束循环_R3三职能协同版.md`。
...
下面各工作块已经给出这些字段。若代码事实与本文不一致，应先在 `docs/R3-min推进记录.md` 记录偏差，再按当前代码事实修订任务，不得默默改变架构边界。
...
- 交付物：基线提交或明确的未提交改动清单；`docs/R3-min推进记录.md` 中的起点记录。
...
1. 读取最新 `R3GateReport` 和 `docs/R3-min推进记录.md`。
