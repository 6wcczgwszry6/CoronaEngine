...
class VISION_LIGHT_PT_light(bpy.types.Panel, VisionWidget):
    ...
    def draw(self, context):
        ...
        if light.type == "SPOT":
            col.prop(light, "vision_light_type", text="Vision Type")
...
def register():
    bpy.types.Light.vision_light_type = EnumProperty(
        name="Vision Type",
        description="Vision light type exported for Blender spot lights",
        items=(
            ("spot", "Spot", "Export as a Vision spot light"),
            ("projector", "Projector", "Export as a Vision projector light"),
        ),
        default="spot",
    )
...
def unregister():
    del bpy.types.Light.vision_light_type
