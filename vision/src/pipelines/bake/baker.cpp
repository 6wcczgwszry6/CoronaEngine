...
void Baker::_compile_bake() noexcept {
    ...
    TIntegrator &integrator = renderer().integrator();
    Kernel kernel = [&](Uint frame_index, BufferVar<Triangle> triangles,
        ...
        sampler->set_seed(dispatch_idx().xy(), frame_index, 0);
        ...
        Float3 L = integrator->Li(rs, scatter_pdf, renderer().spectrum()->one(), it, render_env) * weight;
...
void Baker::_save_result(ocarina::span<BakedShape> baked_shapes) noexcept {
    ...
    for (BakedShape &bs : baked_shapes) {
        ...
        stream() << bs.lightmap_tex().copy_from_buffer(final_radiance_, offset);
        stream() << bs.save_lightmap_to_cache();
        stream() << synchronize() << commit();
        pipeline()->bindless_array().emplace(bs.lightmap_tex());
        offset += bs.pixel_num();
    }
}
...
CommandBatch Baker::clear() noexcept {
    CommandBatch ret;
    ret << radiance_.reset();
...
CommandBatch Baker::deallocate() noexcept {
    CommandBatch ret;
