...
void BakePipeline::init_project(const vision::ProjectDesc &project_desc) {
...
void BakePipeline::prepare() noexcept {
    ...
    lightmap_base_index_ = pipeline()->bindless_array().texture3d_num();
    bake_all();
    update_geometry();
    upload_bindless_array();
}
...
void BakePipeline::compile_displayer() noexcept {
    ...
    TSampler &sampler = renderer_.sampler();
    Kernel kernel = [&](Uint frame_index, Uint lightmap_base) {
        ...
        $if(it.has_lightmap()) {
            Float4 t = bindless_array().tex3d_var(lightmap_base + it.lightmap_id).sample(4, it.lightmap_uv).as_vec4();
            L = t.xyz() / t.w;
            $if(has_invalid(L)) {
                L = make_float3(0.f);
            };
...
void BakePipeline::bake_all() noexcept {
    ...
    baker_stats_.set_spp(renderer_.sampler()->sample_per_pixel());
