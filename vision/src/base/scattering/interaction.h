...
public:
    void set_frame(const Frame<T> &frame) {
        this->x = frame.x;
        this->y = frame.y;
        this->z = frame.z;
    }
    void update(const vec_ty &norm) noexcept {
        auto is_right = Super::is_right_hand();
        this->z = norm;
        vec_ty new_x = normalize(cross(this->y, norm)) * length(this->x);
        this->x = ocarina::select(is_right, new_x, -new_x);
        vec_ty new_y = normalize(cross(norm, this->x)) * length(this->y);
        this->y = ocarina::select(is_right, new_y, -new_y);
    }
...
#define VS_HALF_RADIANCE 0

#if VS_HALF_RADIANCE

using RadType = half;
using RadTypeVar = Var<RadType>;

using RadType4 = Vector<RadType, 4>;
using RadType4Var = Var<RadType4>;

using RadType3 = Vector<RadType, 3>;
using RadType3Var = Var<RadType3>;

template<typename... Args>
auto make_RadType4(Args &&...args) noexcept {
    return make_half4(std::forward<Args>(args)...);
}

template<typename... Args>
auto make_RadType3(Args &&...args) noexcept {
    return make_half3(std::forward<Args>(args)...);
}
