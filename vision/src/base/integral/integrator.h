...
//    }
//}
class Integrator : public Node, public GUIRenderable, public EncodedObject {
public:
...
protected:
    ...
    mutable IntegratorStageProfile cur_stage_profile_{};
...
public:
    ...
    explicit Integrator(const IntegratorDesc &desc)
        : Node(desc) {}
    VS_HOTFIX_MAKE_RESTORE(Node, frame_index_, render_time_, cur_render_time_, cur_stage_profile_, shader_, datas_)
    ...
    [[nodiscard]] const IntegratorStageProfile &cur_stage_profile() const noexcept { return cur_stage_profile_; }
...
struct PTParam {
    ...
    uint enable_sparse_sampling{0u};
...
OC_PARAM_STRUCT(vision, PTParam, frame_index, dimension_index, enable_sparse_sampling, rays, colors, indirect, direct, sampling_mask){};
...
public:
    ...
    void set_denoise_enabled(bool enabled) noexcept override {
        if (denoiser_) {
            denoiser_->set_enabled(enabled);
        }
    }
    ...
    [[nodiscard]] CommandBatch path_tracing(const PTParam &param, uint2 res) const noexcept;
    ...
    [[nodiscard]] CommandBatch denoise(RealTimeDenoiseInput &input) const noexcept;
