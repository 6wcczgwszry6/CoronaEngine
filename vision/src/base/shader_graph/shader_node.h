...
protected:
    ...
    ocarina::vector<SlotInfo> slot_infos_;

protected:
    ShaderNodeSlotSet() = default;
    [[nodiscard]] const ShaderNodeSlot &get_slot(int index) const noexcept;
    [[nodiscard]] size_t slot_num() const noexcept;
    [[nodiscard]] ShaderNodeSlot &get_slot(int index) noexcept;
    void register_slot(const ShaderNodeSlot &slot) noexcept;
...
class ShaderNode : public Node, public GUIRenderable, public Encodable, public enable_shared_from_this<ShaderNode> {
protected:
...
protected:
    ...
    void validate_params(const ShaderNodeDesc &desc, const ParamSchema &schema) const noexcept;
