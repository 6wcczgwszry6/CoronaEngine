...
class Node : public RuntimeObject, public DirtyTrackable, public Hashable {
protected:
    string name_;
...
template<typename TElm, typename TDesc = typename TElm::Desc>
class TObject : public TSlot<SP<TElm>> {
public:
    using element_type = TElm;
    using desc_type = TDesc;
    using Super = TSlot<SP<TElm>>;

public:
    string name;
    TObject() = default;
    TObject(SP<element_type> sp) : Super(ocarina::move(sp)) {}
    explicit TObject(const TDesc &desc) : Super(Node::create_shared<element_type>(desc)) {}
    void init(const TDesc &desc) noexcept { Super::impl_ = Node::create_shared<element_type>(desc); }
    void init(SP<element_type> sp) { Super::impl_ = ocarina::move(sp); }

    template<class U>
    requires std::is_base_of_v<element_type, U>
    TObject(const TObject<U, desc_type> &other) {
    ...
    template<class U>
    requires std::is_base_of_v<element_type, U>
    TObject &operator=(const TObject<U, desc_type> &other) {
        Super::impl_ = other.impl();
        name = other.name;
        return *this;
    }
...
template<typename TElm, typename TDesc = TElm::Desc>
class TObjectUI : public TObject<TElm, TDesc>, public GUI {
public:
    using TObject<TElm, TDesc>::TObject;
    using Super = TObject<TElm, TDesc>;
...
#define VS_MAKE_CLASS_CREATOR_HOTFIX_DIRECTORY(NS, Class, level) \
    VS_REGISTER_HOTFIX(NS, Class)                                \
    VS_MAKE_CLASS_CREATOR(NS::Class)                             \
    VS_REGISTER_DIRECTORY(level)
