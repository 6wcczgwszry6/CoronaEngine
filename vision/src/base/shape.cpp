...
bool ShapeInstance::render_UI(Widgets *widgets) noexcept {
    if (has_material()) {
...
void ShapeInstance::render_sub_UI(Widgets *widgets) noexcept {
    ...
    float4x4 new_o2w = translation<H>(t) * make_float4x4(r.to_float3x3()) * scale<H>(s);
}
...
CommandBatch Mesh::upload() noexcept {
    CommandBatch ret;
