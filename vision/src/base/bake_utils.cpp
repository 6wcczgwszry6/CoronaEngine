...
void BakedShape::save_to_cache(const UnwrapperResult &result) {
    RHIContext::create_directory_if_necessary(cache_directory());
    ...
    RHIContext::write_file(uv_config, data_str);
}

CommandBatch BakedShape::save_lightmap_to_cache() const {
    RHIContext::create_directory_if_necessary(instance_cache_directory());
    CommandBatch ret;
...
CommandBatch BakedShape::save_rasterize_map_to_cache() const {
    CommandBatch ret;
    RHIContext::create_directory_if_necessary(instance_cache_directory());
    float4 *ptr = ocarina::allocate<float4>(pixel_num());
...
void BakedShape::allocate_lightmap_texture() noexcept {
    lightmap_tex_ = device().create_texture3d(resolution(), ocarina::PixelStorage::FLOAT4);
}
