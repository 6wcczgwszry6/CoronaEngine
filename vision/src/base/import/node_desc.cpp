...
void TransformDesc::init(const ParameterSet &ps) noexcept {
    ...
    if (sub_type == "look_at") {
        ...
        mat = ocarina::look_at<EPort::H>(position, target_pos, up);
    } else if (sub_type == "Euler") {
        float4x4 yaw_t = ocarina::rotation_y<EPort::H>(param["yaw"].as_float(0.f), false);
        float4x4 roll_t = ocarina::rotation_z<EPort::H>(param["roll"].as_float(0.f), false);
        float4x4 pitch_t = ocarina::rotation_x<EPort::H>(param["pitch"].as_float(0.f), false);
        float4x4 tt = ocarina::translation<EPort::H>(param["position"].as_float3(make_float3(0.f)));
...
void DenoiserDesc::init(const vision::ParameterSet &ps) noexcept {
    NodeDesc::init(ps);
    sub_type = ps["type"].as_string("svgf");  // 改用 SVGF denoiser
    ParameterSet param = ps.value("param", DataWrap::object());
