...
Kernel variance_kernel = [&](Var<VarianceEstimatorParam> param) {
    ...
    $if(!PixelStateUtils::is_sky(cur_hit)) {
        ...
        Interaction cur_it = geometry_ref.compute_surface_interaction(cur_hit, false);
        ...
        auto check_tap_consistency = [&](Int2 tap_pixel, Float bilinear_w) {
            $if(bilinear_w > 0.001f && 
                ...
                $if(!tap_is_sky) {
                    Interaction tap_it = geometry_ref.compute_surface_interaction(tap_hit, false);
                    Float tap_depth = length(tap_it.pos - param.prev_camera_pos.as_vec3());
                    Bool tap_is_emissive = PixelStateUtils::is_emissive(geometry_ref, tap_hit);
