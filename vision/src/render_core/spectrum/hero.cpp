...
public:
    ...
    [[nodiscard]] const coefficient_table_type &coefficients() const { return *coefficients_; }
    [[nodiscard]] coefficient_table_type &coefficients() { return *coefficients_; }

    void init() noexcept {
        coefficient0_ = rp_->device().create_texture3d(make_uint3(res), PixelStorage::FLOAT4, "RGBToSpectrumTable0");
        coefficient1_ = rp_->device().create_texture3d(make_uint3(res), PixelStorage::FLOAT4, "RGBToSpectrumTable1");
        coefficient2_ = rp_->device().create_texture3d(make_uint3(res), PixelStorage::FLOAT4, "RGBToSpectrumTable2");
    }

    void prepare() noexcept {
        base_index_ = rp_->bindless_array().emplace(coefficient0_);
        rp_->bindless_array().emplace(coefficient1_);
        rp_->bindless_array().emplace(coefficient2_);
    ...
    [[nodiscard]] Float4 decode_albedo(const Float3 &rgb_in) const noexcept {
        ...
        static CALLABLE_TYPE decode = [](Var<BindlessArray> array, Uint base_index, Float3 rgb) noexcept -> Float3 {
            ...
            $if(!(rgb[0] == rgb[1] & rgb[1] == rgb[2])) {
                ...
                c = array.tex3d_var(base_index + maxc).sample(4, coord).as_vec4().xyz();
            };
            return c;
        };
