...
/**
 ...
 In cbox-style scenes, the same object can also be placed under
 shape.param.emission instead of scene.lights.
 */
class AreaLight final : public IAreaLight {
private:
...
public:
    ...
    [[nodiscard]] LightEvalContext sample_surface(Float2 u, Uint prim_id, Float pmf) const noexcept {
        ...
        auto rp = pipeline();
        LightEvalContext p_light = rp->geometry().compute_light_eval_context(*inst_idx_, prim_id, bary);
        p_light.PDF_pos *= pmf;
        return p_light;
    }
    ...
    [[nodiscard]] LightEvalContext compute_light_eval_context(const LightSampleContext &p_ref,
        ...
        auto rp = pipeline();
        return rp->geometry().compute_light_eval_context(*inst_idx_, lsp.prim_id, lsp.bary);
    }
