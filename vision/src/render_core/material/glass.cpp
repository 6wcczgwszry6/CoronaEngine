...
private:
#define GLASS_SLOTS(X)                                       \
    X(color, make_float3(1.f), Albedo, 3u, false)           \
    X(ior, 1.5f, Number, 1u, false)                         \
    X(roughness, 0.5f, Number, 1u, false)                   \
    X(anisotropic, 0.f, Number, 1u, false)

#define GLASS_DECLARE_SLOT_(name, val, tag, dim, required) VS_MAKE_SLOT(name)
    GLASS_SLOTS(GLASS_DECLARE_SLOT_)
#undef GLASS_DECLARE_SLOT_

    [[nodiscard]] static const ParamSchema &param_schema() noexcept {
        static const ParamSchema schema = [] {
            ParamSchema ret;
#define GLASS_REGISTER_PARAM_(name, val, tag, dim, required) ret.add_slot(#name, tag, dim, required);
            GLASS_SLOTS(GLASS_REGISTER_PARAM_)
#undef GLASS_REGISTER_PARAM_
