...
private:
#define SUBSTRATE_SLOTS(X)                                          \
    X(color, make_float3(1.f), Albedo, , 3u, false)                 \
    X(spec, make_float3(0.05f), Albedo, , 3u, false)                \
    X(roughness, 0.5f, Number, .set_range(0.0001f, 1.f), 1u, false) \
    X(anisotropic, 0.f, Number, .set_range(-1, 1), 1u, false)

#define SUBSTRATE_DECLARE_SLOT_(name, val, tag, extra, dim, required) VS_MAKE_SLOT(name)
    SUBSTRATE_SLOTS(SUBSTRATE_DECLARE_SLOT_)
#undef SUBSTRATE_DECLARE_SLOT_

    [[nodiscard]] static const ParamSchema &param_schema() noexcept {
        static const ParamSchema schema = [] {
            ParamSchema ret;
#define SUBSTRATE_REGISTER_PARAM_(name, val, tag, extra, dim, required) ret.add_slot(#name, tag, dim, required);
            SUBSTRATE_SLOTS(SUBSTRATE_REGISTER_PARAM_)
#undef SUBSTRATE_REGISTER_PARAM_
...
#define SUBSTRATE_INIT_SLOT_(name, val, tag, extra, dim, required) VS_INIT_SLOT(name, val, tag) extra;
        SUBSTRATE_SLOTS(SUBSTRATE_INIT_SLOT_)
#undef SUBSTRATE_INIT_SLOT_
    }
    ...
    [[nodiscard]] UP<Lobe> create_lobe_set(const Interaction &it, const SampledWavelengths &swl) const noexcept override {
        ...
        return make_unique<SubstrateLobe>(fresnel, std::move(refl), SurfaceData::Glossy, shading_frame);
    }
};
