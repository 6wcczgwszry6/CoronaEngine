...
/**
 Example material JSON:
 {
     ...
     "param" : {
         ...
         "subsurface_weight" : { "channels" : "x", "node" : { "type" : "number", "param" : { "value" : 0.3 } } },
 ...
 */
class PrincipledBSDF final : public Material {
public:
...
// X-macro table: (name, default_value, tag, extra, dim, required)
// 'extra' is applied after VS_INIT_SLOT, e.g. .set_range(...)
#define PRINCIPLED_SLOTS(X)                                              \
    X(color, make_float3(1.f), Albedo, , 3u, false)                      \
    X(metallic, 0.f, Number, , 1u, false)                                \
    X(ior, 1.5f, Number, .set_range(1.01, 20.f), 1u, false)              \
    X(roughness, 0.5f, Number, .set_range(0.0001f, 1.f), 1u, false)      \
    X(spec_tint, make_float3(1.f), Albedo, , 3u, false)                  \
    X(anisotropic, 0.f, Number, , 1u, false)                             \
    X(opacity, 1.f, Number, .set_range(0.f, 1.f), 1u, false)             \
    X(sheen_weight, 0.f, Number, , 1u, false)                            \
    X(sheen_roughness, 0.5f, Number, , 1u, false)                        \
    X(sheen_tint, make_float3(1.f), Albedo, , 3u, false)                 \
    X(coat_weight, 0.f, Number, , 1u, false)                             \
    X(coat_roughness, 0.2f, Number, .set_range(0.0001f, 1.f), 1u, false) \
    X(coat_ior, 1.5f, Number, .set_range(1.01, 4.f), 1u, false)          \
    X(coat_tint, make_float3(1.f), Albedo, , 3u, false)                  \
    X(subsurface_weight, 0.3f, Number, , 1u, false)                      \
    X(subsurface_radius, make_float3(1.f), Number, , 3u, false)          \
    X(subsurface_scale, 0.2f, Number, , 1u, false)                       \
    X(transmission_weight, 0.f, Number, , 1u, false)

#define PRINCIPLED_DECLARE_SLOT_(name, val, tag, extra, dim, required) VS_MAKE_SLOT(name)
    PRINCIPLED_SLOTS(PRINCIPLED_DECLARE_SLOT_)
#undef PRINCIPLED_DECLARE_SLOT_

    [[nodiscard]] static const ParamSchema &param_schema() noexcept {
        static const ParamSchema schema = [] {
            ParamSchema ret;
#define PRINCIPLED_REGISTER_PARAM_(name, val, tag, extra, dim, required) ret.add_slot(#name, tag, dim, required);
            PRINCIPLED_SLOTS(PRINCIPLED_REGISTER_PARAM_)
#undef PRINCIPLED_REGISTER_PARAM_
            return ret;
        }();
        return schema;
    }
...
#define PRINCIPLED_INIT_SLOT_(name, val, tag, extra, dim, required) VS_INIT_SLOT(name, val, tag) extra;
        PRINCIPLED_SLOTS(PRINCIPLED_INIT_SLOT_)
#undef PRINCIPLED_INIT_SLOT_
    }
