...
_IMPLICIT_COMMAND_PATTERNS = [
    ...
    r"(?:缩小|放大|旋转|调整|修改|改正).{1,15}",     # 无"把"前缀的修改类
    r"(?:往|向)(?:左|右|前|后|上|下).{0,5}(?:移|挪|搬|推|拉)",  # "往左边移动1m"等
    ...
    r"一键(?:生成|布置|组合)",
    # 场景查询 + 相机控制（CAIApp 原生 agent 已有全套工具）
    r"(?:场景|房间)(?:里|内|中).{0,6}(?:有|是)(?:什么|哪些|几个)",
    r"(?:列出|查看|看下|显示).{0,6}(?:模型|物体|场景|家具)",
    r"聚焦.{1,10}", r"拍(?:个|张)?照", r"截图", r"截个图",
    r"(?:转|切换|调整).{0,5}(?:角度|视角|相机|摄像头)",
    r"从.{1,10}(?:看|视角|角度)",
    r"导入.{1,50}\.(?:obj|glb|gltf|fbx|dae|stl)",
]
