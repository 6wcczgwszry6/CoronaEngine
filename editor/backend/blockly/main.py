    ...
    def _notify_preview_state_restored(scene_routes: set[str]) -> None:
        try:
            ...
            for route in scene_routes or {selected_scene or ""}:
                CoronaEditor.emit_editor_event("scene-tree-changed", [route])
            if selected_scene and selected_actor:
                CoronaEditor.emit_editor_event("actor-change", ["actor", selected_scene, selected_actor])
            elif selected_scene:
                CoronaEditor.emit_editor_event("actor-change", ["scene", selected_scene, ""])
        except Exception:
