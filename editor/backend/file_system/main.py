    ...
    def rename_item(old_path: str, new_name: str) -> bool:
        ...
        try:
            ...
            if old_path.endswith(".scene"):
                ...
                CoronaEditor.emit_editor_event("scene-rename", [old_path, new_path, scene.name])
                CoronaEditor.emit_editor_event("actor-change", ['scene', new_path, "", old_path])
                ...
                CoronaEditor.emit_editor_event("actor-change", ['actor', '',  new_path, old_path])
            return True
        except Exception as e:
    ...
    def open_file(path: str, file_type: str):
        try:
            if file_type == "scene":
                ...
                CoronaEditor.emit_editor_event("scene-add", [scene.name, scene.route])
                ...
                CoronaEditor.emit_editor_event("actor-change", ['actor', "", actor.route])
            else:
                logger.error(f"No file type: {file_type}")
                return False
            return True
