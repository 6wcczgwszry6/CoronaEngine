...
# 静默模式开关：scratch 脚本执行期间设为 True，抑制所有 print
_quiet = False


def set_quiet(quiet: bool):
    """设置静默模式（True=不打印，用于积木脚本执行期间）"""
    global _quiet
    _quiet = quiet


def _log(msg: str):
    """统一输出：非静默模式用 print，静默模式完全略过"""
    if _quiet:
        return  # 彻底静默，不 print 也不 log
...
class Geometry:
    def __init__(self, model_path: str):
        _log(f"[Fallback][Geometry.__init__] model_path={model_path}")
    ...
    def set_position(self, pos: List[float]):
        _log(f"[Fallback][Geometry.set_position] pos={pos}")
        self._pos = list(pos)

    def set_rotation(self, euler: List[float]):
        _log(f"[Fallback][Geometry.set_rotation] euler={euler}")
        self._rot = list(euler)

    def set_scale(self, scl: List[float]):
        _log(f"[Fallback][Geometry.set_scale] scl={scl}")
        self._scl = list(scl)

    def get_position(self) -> List[float]:
        _log("[Fallback][Geometry.get_position]")
        return list(self._pos)

    def get_rotation(self) -> List[float]:
        _log("[Fallback][Geometry.get_rotation]")
        return list(self._rot)

    def get_scale(self) -> List[float]:
        _log("[Fallback][Geometry.get_scale]")
        return list(self._scl)

    def get_aabb(self) -> List[float]:
        _log("[Fallback][Geometry.get_aabb]")
...
class Mechanics:
    def __init__(self, geo: Geometry):
        _log(f"[Fallback][Mechanics.__init__] geo={geo}")
        self._geo = geo
        self._mass = 1.0
...
class Optics:
    def __init__(self, geo: Geometry):
        _log(f"[Fallback][Optics.__init__] geo={geo}")
        self._geo = geo
        self._metallic = 0.0
        self._roughness = 0.5
...
class Acoustics:
    def __init__(self, geo: Geometry):
        _log(f"[Fallback][Acoustics.__init__] geo={geo}")
        self._geo = geo
        self._volume = 1.0

    def set_volume(self, volume: float):
        _log(f"[Fallback][Acoustics.set_volume] volume={volume}")
        self._volume = float(volume)

    def get_volume(self) -> float:
        _log("[Fallback][Acoustics.get_volume]")
        return float(self._volume)


class Kinematics:
    def __init__(self, geo: Geometry):
        _log(f"[Fallback][Kinematics.__init__] geo={geo}")
        self._geo = geo
        self._anim_index = 0
        self._time = 0.0
        self._playing = False
    ...
    def set_animation(self, index: int):
        _log(f"[Fallback][Kinematics.set_animation] index={index}")
        self._anim_index = int(index)
        self._time = 0.0

    def play_animation(self, speed: float = 1.0):
        _log(f"[Fallback][Kinematics.play_animation] speed={speed}")
        self._playing = True
        self._speed = float(speed)

    def stop_animation(self):
        _log("[Fallback][Kinematics.stop_animation]")
        self._playing = False

    def get_animation_index(self) -> int:
        _log("[Fallback][Kinematics.get_animation_index]")
        return int(self._anim_index)

    def get_current_time(self) -> float:
        _log("[Fallback][Kinematics.get_current_time]")
...
class ActorProfile:
    def __init__(self):
        _log("[Fallback][ActorProfile.__init__]")
...
class Actor:
    def __init__(self, path: str = ""):
        _log(f"[Fallback][Actor.__init__] path={path}")
        # 兼容旧构造签名（path），但不再使用
    ...
    def add_profile(self, profile: ActorProfile) -> Optional[ActorProfile]:
        ...
        if geo is None:
            _log("[Fallback][Actor.add_profile] profile.geometry 不能为空")
            return None
        for comp in (profile.optics, profile.mechanics, profile.acoustics, profile.kinematics):
            if comp is not None and getattr(comp, "_geo", None) is not geo:
                _log("[Fallback][Actor.add_profile] 组件与 profile.geometry 不一致，拒绝添加")
                return None
    ...
    def remove_profile(self, profile: ActorProfile):
        _log(f"[Fallback][Actor.remove_profile] profile={profile}")
    ...
    def set_active_profile(self, profile: ActorProfile):
        _log(f"[Fallback][Actor.set_active_profile] profile={profile}")
        if profile in self._profiles:
            self._active = profile
        else:
            _log("[Fallback][Actor.set_active_profile] 指定的 profile 不属于该 Actor")

    def get_active_profile(self) -> Optional[ActorProfile]:
        _log("[Fallback][Actor.get_active_profile]")
        return self._active
...
class Camera:
    ...
    def set_surface(self, surface):
        _log(f"[Fallback][Camera.set_surface] surface={surface}")
    ...
    def get_forward(self):
        _log("[Fallback][Camera.get_forward]")
        return list(self._forward)

    def get_world_up(self):
        _log("[Fallback][Camera.get_world_up]")
        return list(self._world_up)

    def get_fov(self):
        _log("[Fallback][Camera.get_fov]")
        return float(self._fov)

    def save_screenshot(self, path: str):
        _log(f"[Fallback][Camera.save_screenshot] path={path}")

    def set_output_mode(self, mode: str):
        _log(f"[Fallback][Camera.set_output_mode] mode={mode}")
    ...
    def get_image_effects(self):
        _log("[Fallback][Camera.get_image_effects]")
        return self._effects

    def has_image_effects(self) -> bool:
        _log("[Fallback][Camera.has_image_effects]")
        return self._effects is not None

    def remove_image_effects(self):
        _log("[Fallback][Camera.remove_image_effects]")
        self._effects = None
    ...
    def pick_actor_at_pixel(self, x: int, y: int):
        _log(f"[Fallback][Camera.pick_actor_at_pixel] x={x}, y={y}")
        return 0


class ImageEffects:
    def __init__(self):
        _log("[Fallback][ImageEffects.__init__]")
...
class Environment:
    ...
    def set_floor_y(self, y: float):
        _log(f"[Fallback][Environment.set_floor_y] y={y}")
    ...
    def set_floor_restitution(self, restitution: float):
        _log(f"[Fallback][Environment.set_floor_restitution] restitution={restitution}")
    ...
    def set_fixed_dt(self, dt: float):
        _log(f"[Fallback][Environment.set_fixed_dt] dt={dt}")
...
class Scene:
    def __init__(self, light_field: bool = False):
        _log(f"[Fallback][Scene.__init__] light_field={light_field}")
    ...
    def set_environment(self, env: Optional[Environment]):
        _log(f"[Fallback][Scene.set_environment] env={env}")
        self._env = env

    def get_environment(self) -> Optional[Environment]:
        _log("[Fallback][Scene.get_environment]")
        return self._env

    def has_environment(self) -> bool:
        _log("[Fallback][Scene.has_environment]")
        return self._env is not None

    def remove_environment(self):
        _log("[Fallback][Scene.remove_environment]")
        self._env = None

    # Actor
    def add_actor(self, actor: Actor):
        _log(f"[Fallback][Scene.add_actor] actor={actor}")
        if actor not in self._actors:
            self._actors.append(actor)

    def remove_actor(self, actor: Actor):
        _log(f"[Fallback][Scene.remove_actor] actor={actor}")
        if actor in self._actors:
            self._actors.remove(actor)

    def clear_actors(self):
        _log("[Fallback][Scene.clear_actors]")
        self._actors.clear()

    def actor_count(self) -> int:
        _log("[Fallback][Scene.actor_count]")
        return len(self._actors)

    def has_actor(self, actor: Actor) -> bool:
        _log(f"[Fallback][Scene.has_actor] actor={actor}")
        return actor in self._actors

    # Camera (scene-level)
    def add_camera(self, cam: Camera):
        _log(f"[Fallback][Scene.add_camera] camera={cam}")
    ...
    def clear_cameras(self):
        _log("[Fallback][Scene.clear_cameras]")
        self._cameras.clear()

    def camera_count(self) -> int:
        _log("[Fallback][Scene.camera_count]")
        return len(self._cameras)

    def has_camera(self, cam: Camera) -> bool:
        _log(f"[Fallback][Scene.has_camera] camera={cam}")
        return cam in self._cameras

    def get_aabb(self):
        _log("[Fallback][Scene.get_aabb]")
        return [0, 0, 0, 0, 0, 0]
