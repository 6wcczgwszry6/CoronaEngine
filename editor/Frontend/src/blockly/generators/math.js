...
export const defineMathGenerators = () => {
  ...
  pythonGenerator.forBlock.math_connect = function (block) {
    ...
    return [`str(${left}) + str(${right})`, pythonGenerator.ORDER_ADDITIVE];
  };

  const operand = (block, input, legacy, fallback = '0') => (
    pythonGenerator.valueToCode(block, input, pythonGenerator.ORDER_NONE)
    || block.getFieldValue(legacy)
    || fallback
  );
  const binary = (operator, order) => (block) => [
    `(${operand(block, 'A', 'x1')} ${operator} ${operand(block, 'B', 'x2')})`,
    order,
  ];
  ...
  pythonGenerator.forBlock.math_random = (block) => [
    `CoronaEngine.random(${operand(block, 'A', 'x1')}, ${operand(block, 'B', 'x2', '10')})`,
